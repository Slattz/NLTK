...
bool VerifyCRC32(u32 crc, u8 *buf, u32 startOffset, u32 size) {
	return CalculateCRC32(buf + startOffset + 4, size) == crc;
}

u32 UpdateCRC32(Save *save, u32 startOffset, u32 size) {
	u32 crc32 = CalculateCRC32(save->GetRawSaveData() + startOffset + 4, size); // calculate the crc32 for the region
...
void FixCRC32s(Save *save) {
	UpdateCRC32(save, 0x80, 0x1C);

	// Rehash players
	for (int i = 0; i < 4; i++) {
		UpdateCRC32(save, 0xA0 + (0xA480 * i), 0x6B84);
		UpdateCRC32(save, 0xA0 + (0xA480 * i) + 0x6B88, 0x38F4);
	}

	UpdateCRC32(save, 0x292A0, 0x22BC8);
	UpdateCRC32(save, 0x4BE80, 0x44B8);
	UpdateCRC32(save, 0x53424, 0x1E4D8);
	UpdateCRC32(save, 0x71900, 0x20);
	UpdateCRC32(save, 0x71924, 0xBE4);
	UpdateCRC32(save, 0x73954, 0x16188);
}
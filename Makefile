...
LIBS	:=  -lfreetype -lpng -lbz2 -lcurl -lmbedtls -lmbedx509 -lmbedcrypto -lz -lturbojpeg -lcitro2d -lcitro3d -lctru -lm
...
#---------------------------------------------------------------------------------
	@echo $(notdir $<)
	@tex3ds -i $< -H $(TOPDIR)/include/sprites/$*.h -d $*.d -o $(TOPDIR)/$(GFXBUILD)/$*.t3x
